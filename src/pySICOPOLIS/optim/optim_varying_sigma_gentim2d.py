# NOTE: THE CORRESPONDING CODES FOR THIS ARE optim_AB_nodeltats_noconvinit_constrained_{28,29,30,31,35}.py and sicopolis_optim_AB_nodeltats_noconvinit_constrained_{28,29,30,31,35}
...
class DataAssimilation:
    ...
    def __init__(self,
        ...
        if any(value == "2DT" for value in self.dict_params_fields_num_dims.values()):
            ...
            self.ds_prior_fields["gentim2d_sigma_arr"] = xr.DataArray(np.atleast_2d(list(self.dict_prior_sigmas_2dt.values())), dims=["gentim2d", "gentim2d_ntdamax_plus_1"], attrs={"type": "hyperparameter_prior"})
    ...
    def eval_sqrt_prior_cov_inv_action(self) -> Any:
        ...
        for var in ds_subset_x:
            ...
            if not self.list_fields_to_ignore or (self.list_fields_to_ignore and var[:-1] not in self.list_fields_to_ignore):
                ...
                elif self.dict_params_fields_num_dims[basic_str] == "2DT":
                    dict_tlm_action_only_fields_vals[var] = ds_subset_x[var].data.copy() / (self.dict_prior_sigmas[basic_str][:, None, None] * self.ds_prior_X[var].data)
                else:
    ...
    def eval_sqrt_prior_cov_action(self,
        ...
        for var in ds_subset_fields_adj_or_tlm_action:
            ...
            if (not self.list_fields_to_ignore or (self.list_fields_to_ignore and basic_str not in self.list_fields_to_ignore)):
                if self.dict_params_fields_num_dims[basic_str] != "2DT":
                    ...
                    ds_subset_fields_adj_or_tlm_action[var].data = ds_subset_fields_adj_or_tlm_action[var].data * self.dict_prior_sigmas[basic_str][:, None, None] * self.ds_prior_X[basic_str + "d"].data
        ...
        for varb in ds_subset_adj_fields:
            ...
            if not self.list_fields_to_ignore or (self.list_fields_to_ignore and basic_str not in self.list_fields_to_ignore):
                if self.dict_params_fields_num_dims[basic_str] != "2DT":
                    ...
                    ds_subset_adj_fields[varb].data = ds_subset_adj_fields[varb].data * self.dict_prior_sigmas[basic_str][:, None, None] * self.ds_prior_X[basic_str + "d"].data
